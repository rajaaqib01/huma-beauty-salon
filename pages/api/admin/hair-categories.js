...
const DB_NAME = 'hair_categories'
const TABLE_NAME = 'hair_categories'
  ...
  if (!supabaseServer) {
    if (req.method === 'GET') {
      try {
        ...
        console.error('Local hair categories load error:', e)
  ...
  if (req.method === 'POST') {
    ...
    if (!supabaseServer) {
      try {
        ...
        console.error('Local hair category insert error:', e)
        return res.status(500).json({ error: e.message })
      }
    }
  ...
  if (req.method === 'PUT') {
    ...
    if (!supabaseServer) {
      try {
        ...
        console.error('Local hair category update error:', e)
        return res.status(500).json({ error: e.message })
      }
    }
    const { data, error } = await supabaseServer
  ...
  if (req.method === 'DELETE') {
    ...
    if (!supabaseServer) {
      try {
        ...
        console.error('Local hair category delete error:', e)
        return res.status(500).json({ error: e.message })
      }
    }

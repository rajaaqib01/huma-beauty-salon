...
const DB_NAME = 'waxing_categories'
const TABLE_NAME = 'waxing_categories'
  ...
  if (!supabaseServer) {
    if (req.method === 'GET') {
      try {
        ...
        console.error('Local waxing categories load error:', e)
  ...
  if (req.method === 'POST') {
    ...
    if (!supabaseServer) {
      try {
        ...
        console.error('Local waxing category insert error:', e)
        return res.status(500).json({ error: e.message })
      }
    }
  ...
  if (req.method === 'PUT') {
    ...
    if (!supabaseServer) {
      try {
        ...
        console.error('Local waxing category update error:', e)
        return res.status(500).json({ error: e.message })
      }
    }
    const { data, error } = await supabaseServer
  ...
  if (req.method === 'DELETE') {
    ...
    if (!supabaseServer) {
      try {
        ...
        console.error('Local waxing category delete error:', e)
        return res.status(500).json({ error: e.message })
      }
    }

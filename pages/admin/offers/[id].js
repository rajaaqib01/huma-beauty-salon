      ...
      <div className="grid gap-4 max-w-3xl">
        {['title', 'description', 'discount', 'image_url', 'starts_at', 'ends_at'].map((field) => (
          <div key={field}>
            ...
            <input className="w-full rounded-2xl bg-slate-800 border border-white/10 p-3 text-slate-100" value={offer[field] || ''} onChange={(e) => setOffer({ ...offer, [field]: e.target.value })} type={field.includes('at') ? 'date' : 'text'} />
          </div>
        ))}

  ...
  return (
    <AdminShell title="Edit Service">
      <div className="grid gap-4 max-w-3xl">
        ...
        <select value={service.category || ''} onChange={e=>setService({...service, category: e.target.value})} className="w-full rounded-3xl bg-slate-800 border border-white/10 p-4 text-white">
          <option value="">Select Category</option>
          {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
        </select>
        <input value={service.price} onChange={e=>setService({...service, price: e.target.value})} className="w-full rounded-3xl bg-slate-800 border border-white/10 p-4 text-white" placeholder="Price (numbers only, e.g. 3000)" />

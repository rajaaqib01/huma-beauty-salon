  ...
  return (
    <AdminShell title="Create Service">
      <form onSubmit={handleCreate} className="grid gap-4 max-w-3xl">
        ...
        <input placeholder="Category" value={category} onChange={e=>setCategory(e.target.value)} className="w-full rounded-3xl bg-slate-800 border border-white/10 p-4 text-white" />
        <input placeholder="Price" value={price} onChange={e=>setPrice(e.target.value)} className="w-full rounded-3xl bg-slate-800 border border-white/10 p-4 text-white" />

...
export function formatOfferDates(startsAt, endsAt) {
  ...
  const start = parseOfferDate(startsAt)
  const end = parseOfferDate(endsAt)
...
export function getOfferScheduleStatus(offer, now = new Date()) {
  const start = parseOfferDate(offer.starts_at)
  const end = parseOfferDate(offer.ends_at)
  ...
  if (end) {
    ...
    if (now > endOfDay) return { label: 'Expired', tone: 'expired' }
  }

  if (isOfferActive(offer, now)) return { label: 'Live', tone: 'live' }

...
/** Owner-only APIs (gallery, settings, users, etc.) */
export function requireOwnerAdmin(handler) {
  return requireAdmin(async (req, res) => {
    const role = String(req.admin?.role || 'owner').toLowerCase()
    if (role === 'reception') {
      return res.status(403).json({ error: 'Access denied for reception account' })
    }
    return handler(req, res)
  })
}

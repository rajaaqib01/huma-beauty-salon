...
export function filterSales(bookings, { month = '', date = '', service = '', source = '' } = {}) {
  let rows = bookings
    .filter(isConfirmedSale)
    .map(normalizeSaleRow)

  if (date) {
    rows = rows.filter((r) => String(r.date) === String(date))
  } else if (month) {
...
export function buildDaySummary(sales, date) {
  const day = String(date || '')
  const rows = Array.isArray(sales) ? sales : []
  return {
    date: day,
    count: rows.length,
    total: rows.reduce((s, r) => s + (r.sale_amount || 0), 0),
    online: rows.filter((r) => r.source !== 'manual').reduce((s, r) => s + (r.sale_amount || 0), 0),
    manual: rows.filter((r) => r.source === 'manual').reduce((s, r) => s + (r.sale_amount || 0), 0),
  }
}

...
/** Read from Supabase; on missing config or query error, use bundled/local fallback. */
export async function supabaseRead(table, query, fallback) {
  if (!supabaseServer) return fallback()
  const { data, error } = await query(supabaseServer)
  if (!error && data != null) return data
  if (error) console.error(`Supabase ${table} read fallback:`, error.message)
  return fallback()
}
